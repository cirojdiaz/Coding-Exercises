"""
You are given a tree that contains N nodes, each containing an integer u which corresponds
...
def count_of_nodes(root, queries, s):
    ...
    for q in queries:
        valNode = q[0]
        ch = q[1]
        # travel the tree with breath search to find node valNode
        node = FindNode(root, valNode)
        ans.append(countNodes(node, {i + 1 for i, e in enumerate(s) if e == ch}))
    return ans
